    ...
    while (p) {
        ...
        switch (c->kind) {
            ...
            case IR_ADD: case IR_SUB: case IR_MUL: case IR_DIV: {
                ...
                if (c->kind == IR_ADD) emit_add(out, dest, x, y);
                if (c->kind == IR_SUB) emit_sub(out, dest, x, y);
                if (c->kind == IR_MUL) emit_mul(out, dest, x, y);
                if (c->kind == IR_DIV) emit_div(out, dest, x, y);
            ...
            case IR_CALL: {
                ...
                emit_jal(out, c->u.call.func->u.name);  // 正确字段！
                if (strstr(z, "($fp)")) {
            ...
            case IR_IF_GOTO: {
                ...
                sprintf(label, "label%d", c->u.if_goto.label->u.no);  // 正确字段！
                const char *rel = c->u.if_goto.relop;
                if (strcmp(rel, "==") == 0) emit_beq(out, x, y, label);
                else if (strcmp(rel, "!=") == 0) emit_bne(out, x, y, label);
                else if (strcmp(rel, "<")  == 0) emit_blt(out, x, y, label);
                else if (strcmp(rel, ">")  == 0) emit_bgt(out, x, y, label);
                else if (strcmp(rel, "<=") == 0) emit_ble(out, x, y, label);
                else if (strcmp(rel, ">=") == 0) emit_bge(out, x, y, label);
                break;
            }
