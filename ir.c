...
/* 和 tree.c 一样搞个安全 malloc/strdup，风格统一 */
...
void ir_free_all(void) {
    ...
    while (p) {
        ...
        /* 释放 InterCode 里的字符串/操作数等 */
        /* 简化写法：只释放 name/relop，Operand 的共享问题由翻译层自己控制也行。
           如果想“纯净一些”，可以在这里递归释放所有 Operand。 */
        free(p->code);
        free(p);
        p = nxt;
    }
...
static void print_operand(FILE* out, Operand op) {
    ...
    case OP_VARIABLE:
        ...
        break;
    case OP_TEMP:
        fprintf(out, "t%d", op->u.no);
        break;
    case OP_CONSTANT:
        fprintf(out, "#%d", op->u.value);
        break;
    case OP_LABEL:
        fprintf(out, "label%d", op->u.no);
        break;
    case OP_FUNCTION:
        fprintf(out, "%s", op->u.name);
        break;
    case OP_ADDRESS:
        /* 这里简单写成 *base，具体格式按你们实验书调整 */
        fprintf(out, "*");
